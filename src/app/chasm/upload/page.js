...
export default function Upload() {
    ...
    const stdErrMsgHTML = 'There was an error uploading your photo. Please check the file size and&nbsp;type.';

    const fireError = (customMsg) => {
        const errMsg = document.querySelector('.error-message');
        errMsg.innerHTML = customMsg ? customMsg : stdErrMsgHTML;

        // if the error message is already up, just have it flash real quick.
        if (errMsg.classList.contains('err-on')) {
        ...
        errMsg.classList.add('err-on');
        document.querySelector('form#upload button').textContent = buttonText;
    }
        ...
        <main className="flex flex-col min-h-screen items-center justify-center">
            ...
            >
                ...
                <form // action={(e) => handleSubmit(e)}
                    // onFormData={(e) => { console.log('form data')}}
                ...
                >
                    ...
                    />
                    <button type="submit"
                        className="btn px-8 py-2 text-white rounded-[30px]"
                    >
                        {buttonText}
                    </button>
                </form>
                <div className="w-full max-w-[500px] min-h-[50px] relative flex items-center justify-center mx-auto ">
                    <span className="absolute top-0 error-message text-[#731616] font-bold text-center"></span>
